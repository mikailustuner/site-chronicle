      ...
      : next.name === 'audits'
        ...
        : next.name === 'opportunities'
          ...
          : next.name === 'visibility'
            ...
            : next.name === 'growth' ? '/growth'
            : next.name === 'ads' ? '/ads'
            : next.name === 'strategy' ? '/strategy'
                  ...
                  : view.name === 'technical' ? <TechnicalHub go={go} />
                    : view.name === 'performance' ? <PublicPerformanceHub go={go} />
                      : view.name === 'changes' ? <ChangesHub go={go} />
                        : view.name === 'growth' ? <GrowthStudio go={go} />
                          : view.name === 'ads' ? <AdStudio go={go} />
                            : view.name === 'strategy' ? <StrategyLab go={go} />
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Primary navigation">
          ...
          <span className="nav-label">Growth & delivery</span>
          <NavButton active={current === 'growth'} icon="spark" onClick={() => go({ name: 'growth' })}>360° improvement</NavButton>
          <NavButton active={current === 'ads'} icon="megaphone" onClick={() => go({ name: 'ads' })}>Ad strategy studio</NavButton>
          <NavButton active={current === 'strategy'} icon="briefcase" onClick={() => go({ name: 'strategy' })}>Solution desk</NavButton>
...
function GrowthStudio({go}:{go:(view:View)=>void}){
  const [domains,setDomains]=useState<Row[]>([]);const [domainId,setDomainId]=useState('');const [data,setData]=useState<Row|null>(null);
  useEffect(()=>{void api<Row[]>('/api/domains').then(rows=>{setDomains(rows);setDomainId(rows[0]?.id??'')})},[]);
  useEffect(()=>{if(!domainId){setData(null);return}setData(null);void api<Row>(`/api/domains/${domainId}/site-360`).then(setData)},[domainId]);
  if(!domains.length)return <><Header eyebrow="Whole-site operating view" title="Improve the business surface, not just the score." subtitle="Add a site to combine technical, demand, offer, performance, trust and paid-readiness evidence."/><Empty title="No site selected" text="The 360° view needs an authorized public origin."/></>;
  if(!data)return <Loading/>;const pillars=data.pillars as Row[];
  return <><Header eyebrow="360° improvement" title="Improve the whole site as a system." subtitle="Technical eligibility, search demand, offer clarity, experience, trust and paid readiness remain separate—but are prioritized together." action={<SiteScope domains={domains} value={domainId} onChange={setDomainId}/>}/>
    <div className="alert neutral"><b>Decision boundary.</b> {data.boundary}</div>
    <div className="strategy-pillar-grid">{pillars.map(pillar=><article className="panel strategy-pillar" key={pillar.name}><div className="finding-top"><span className={`pill ${pillar.status==='measured'?'ok':'neutral'}`}>{pillar.status}</span><span className="confidence">{pillar.evidenceCount} evidence rows</span></div><h2>{pillar.name}</h2><p>{pillar.interpretation}</p><details><summary>Inspect evidence and current actions</summary><div className="strategy-evidence-list">{(pillar.evidence as Row[]).slice(0,8).map((row,index)=><div key={row.id??index}><b>{row.title??row.query??row.metric??row.category??`Evidence ${index+1}`}</b><small>{row.severity??row.source??row.intent??row.status??''}</small></div>)}</div>{(pillar.opportunities as Row[]).map(row=><div className="schedule-row" key={row.id}><span><b>{row.title}</b><small>{row.recommendation}</small></span><span className="pill neutral">{row.priority}</span></div>)}</details><button onClick={async()=>{const created=await post<Row>(`/api/domains/${domainId}/site-360/case`,{pillar:pillar.name,objective:`Improve ${pillar.name} using the current evidence while protecting all other site guardrails.`});await post(`/api/solution-cases/${created.id}/plan`,{});window.alert('Execution case created in Solution desk.')}}>Create execution case →</button></article>)}</div>
    <section className="panel"><div className="panel-head"><div><p className="eyebrow">Unified backlog</p><h2>Highest-evidence next moves</h2></div><button onClick={()=>go({name:'opportunities'})}>Open opportunity engine →</button></div>{(data.opportunities as Row[]).length?(data.opportunities as Row[]).map(row=><div className="schedule-row" key={row.id}><span className="schedule-glyph"><Icon name="spark"/></span><span><b>{row.title}</b><small>{row.recommendation}</small></span><span className="pill neutral">{row.priority}/100</span></div>):<Empty title="No unified actions yet" text="Run a complete audit and SERP observation, then refresh this view."/>}</section></>;
}

function AdStudio({go}:{go:(view:View)=>void}){
  const [domains,setDomains]=useState<Row[]>([]);const [domainId,setDomainId]=useState('');const [rows,setRows]=useState<Row[]|null>(null);const [channel,setChannel]=useState('google-ads');const [goal,setGoal]=useState('leads');const [offer,setOffer]=useState('');const [audience,setAudience]=useState('');const [geography,setGeography]=useState('Turkey');const [budget,setBudget]=useState(500);const [currency,setCurrency]=useState('TRY');const [destination,setDestination]=useState('');const [busy,setBusy]=useState(false);
  const load=()=>api<Row[]>(`/api/ad-blueprints${domainId?`?domainId=${domainId}`:''}`).then(setRows);useEffect(()=>{void api<Row[]>('/api/domains').then(items=>{setDomains(items);setDomainId(items[0]?.id??'');setDestination(items[0]?.origin??'')})},[]);useEffect(()=>{const domain=domains.find(item=>item.id===domainId);setDestination(domain?.origin??'');setRows(null);void load()},[domainId]);if(!rows)return <Loading/>;
  const create=async(e:FormEvent)=>{e.preventDefault();setBusy(true);try{await post(`/api/domains/${domainId}/ad-blueprints`,{channel,goal,offer,audience,geography,dailyBudget:budget,currency,destinationUrl:destination||undefined,notes:''});setOffer('');await load()}finally{setBusy(false)}};
  return <><Header eyebrow="Access-free media planning" title="Build ads around evidence, not account folklore." subtitle="Create an implementation-ready Google Ads or Meta Ads blueprint from the offer, audience, market, landing page and current site evidence." action={<SiteScope domains={domains} value={domainId} onChange={setDomainId}/>}/>
    <div className="alert warn"><b>Honest limit.</b> Without authorized platform or customer-side conversion data, spend efficiency, CPA, ROAS and sales lift cannot be verified. The studio blocks outcome-optimization claims and designs the strongest measurable starting point.</div>
    <section className="panel"><p className="eyebrow">Campaign brief</p><h2>Generate a channel-specific blueprint</h2><form className="ad-brief-form" onSubmit={create}><label>Channel<select value={channel} onChange={e=>setChannel(e.target.value)}><option value="google-ads">Google Ads</option><option value="meta-ads">Meta Ads</option></select></label><label>Business goal<select value={goal} onChange={e=>setGoal(e.target.value)}>{['leads','sales','traffic','awareness'].map(value=><option key={value}>{value}</option>)}</select></label><label>Daily budget<div className="budget-input"><input type="number" min="1" value={budget} onChange={e=>setBudget(Number(e.target.value))}/><select value={currency} onChange={e=>setCurrency(e.target.value)}>{['TRY','USD','EUR','GBP'].map(value=><option key={value}>{value}</option>)}</select></div></label><label>Geography<input value={geography} onChange={e=>setGeography(e.target.value)} required/></label><label className="wide">Offer<input value={offer} onChange={e=>setOffer(e.target.value)} placeholder="What exactly is being sold or offered? Include real price/qualification only if verified." required/></label><label className="wide">Audience<input value={audience} onChange={e=>setAudience(e.target.value)} placeholder="Who has this problem, in what situation, and who should not qualify?" required/></label><label className="wide">Landing page<input type="url" value={destination} onChange={e=>setDestination(e.target.value)} required/></label><button className="primary" disabled={!domainId||!offer||!audience||busy}>{busy?'Building…':'Build evidence-based plan'}</button></form></section>
    <section className="panel"><p className="eyebrow">Blueprint archive</p><h2>Plans ready for human implementation</h2>{rows.length?<div className="blueprint-list">{rows.map(row=><AdBlueprintCard row={row} onChanged={load}/>)}</div>:<Empty title="No ad blueprints" text="Define the first real offer and audience. The system will not invent either from generic site copy."/>}</section></>;
}

function AdBlueprintCard({row,onChanged}:{row:Row;onChanged:()=>void}){const plan=row.blueprint as Row;const brief=plan.brief??{};return <details className="blueprint-card"><summary><div><span className="category">{row.channel}</span><span className={`pill ${row.status==='validated'?'ok':'neutral'}`}>{row.status}</span><h3>{row.offer}</h3><p>{row.goal} · {row.geography} · {formatNumber(row.daily_budget)} {row.currency}/day</p></div><Icon name="chevron"/></summary><div className="blueprint-body"><div className="privacy-grid"><Fact label="Recommended start" value={plan.recommendedStart??plan.recommendedObjective}/><Fact label="Measurement" value={plan.measurement?.status??'unavailable'}/><Fact label="Destination" value={brief.destinationUrl}/><Fact label="Evidence refs" value={(row.evidence_refs??[]).length}/></div>{plan.campaignStructure&&<StrategySection title="Campaign structure" items={plan.campaignStructure}/>} {plan.keywordSeeds&&<StrategySection title="Keyword seeds — review before use" items={plan.keywordSeeds}/>} {plan.creativeMatrix&&<StrategySection title="Creative matrix" items={plan.creativeMatrix}/>}<StrategySection title="Landing-page checklist" items={plan.landingPage?.checklist??[]}/><StrategySection title="Guardrails" items={plan.guardrails??[]}/><section className="source-strip"><b>Primary platform sources</b>{(row.source_urls??[]).map((url:string)=><a href={url} target="_blank" rel="noreferrer" key={url}>Official guidance ↗</a>)}</section><div className="button-row"><select value={row.status} onChange={async e=>{await patch(`/api/ad-blueprints/${row.id}`,{status:e.target.value});onChanged()}}>{['draft','ready','launched-externally','learning','validated','stopped'].map(value=><option key={value}>{value}</option>)}</select></div></div></details>}
function StrategySection({title,items}:{title:string;items:any[]}){return <section className="strategy-section"><h3>{title}</h3><ol>{items.map((item,index)=><li key={index}>{typeof item==='string'?item:<><b>{item.name??item.angle??item.title??`Item ${index+1}`}</b><span>{Object.entries(item).filter(([key])=>!['name','angle','title'].includes(key)).map(([key,value])=>`${key}: ${String(value)}`).join(' · ')}</span></>}</li>)}</ol></section>}

function StrategyLab({go}:{go:(view:View)=>void}){
  const [data,setData]=useState<Row|null>(null);const [domains,setDomains]=useState<Row[]>([]);const [domainId,setDomainId]=useState('');const [title,setTitle]=useState('');const [area,setArea]=useState('software');const [objective,setObjective]=useState('');const [context,setContext]=useState('');const [priority,setPriority]=useState(3);const [busy,setBusy]=useState(false);
  const load=()=>Promise.all([api<Row>('/api/strategy/overview'),api<Row[]>('/api/domains')]).then(([overview,siteRows])=>{setData(overview);setDomains(siteRows)});useEffect(()=>{void load()},[]);if(!data)return <Loading/>;
  const create=async(e:FormEvent)=>{e.preventDefault();setBusy(true);try{const item=await post<Row>('/api/solution-cases',{domainId:domainId||undefined,title,area,objective,context,constraints:[],priority});await post(`/api/solution-cases/${item.id}/plan`,{});setTitle('');setObjective('');setContext('');await load()}finally{setBusy(false)}};
  const cases=data.cases as Row[];
  return <><Header eyebrow="General problem-solving system" title="Turn any problem into an evidence loop." subtitle="Software, security, automation, growth, product, research and operations work share one disciplined case → plan → action → result workflow."/>
    <div className="metric-grid"><Metric label="Open cases" value={cases.filter(row=>!['validated','closed'].includes(row.status)).length} icon="briefcase"/><Metric label="Actions" value={cases.reduce((sum,row)=>sum+Number(row.actions??0),0)} icon="list"/><Metric label="Completed" value={cases.reduce((sum,row)=>sum+Number(row.completed_actions??0),0)} tone="green" icon="check"/><Metric label="Ad blueprints" value={(data.blueprints as Row[]).length} icon="megaphone"/></div>
    <section className="panel"><p className="eyebrow">New case</p><h2>Frame the next problem</h2><form className="solution-case-form" onSubmit={create}><label>Case title<input value={title} onChange={e=>setTitle(e.target.value)} required placeholder="Automate weekly stock reconciliation"/></label><label>Area<select value={area} onChange={e=>setArea(e.target.value)}>{['software','security','automation','growth','advertising','product','operations','research','other'].map(value=><option key={value}>{value}</option>)}</select></label><label>Related site (optional)<SiteScope domains={domains} value={domainId} onChange={setDomainId}/></label><label>Priority<input type="number" min="0" max="5" value={priority} onChange={e=>setPriority(Number(e.target.value))}/></label><label className="wide">Objective<textarea value={objective} onChange={e=>setObjective(e.target.value)} required placeholder="What state must be true when this is solved?"/></label><label className="wide">Context and constraints<textarea value={context} onChange={e=>setContext(e.target.value)} placeholder="Known facts, deadlines, systems, authority and non-negotiables…"/></label><button className="primary" disabled={!title||!objective||busy}>{busy?'Planning…':'Create case and first plan'}</button></form></section>
    <section className="panel"><div className="panel-head"><div><p className="eyebrow">Active delivery</p><h2>Cases and next actions</h2></div></div>{cases.length?cases.map(row=><SolutionCase key={row.id} row={row} onChanged={load}/>):<Empty title="No cases yet" text="Create a case for any problem—even when it is not tied to a website."/>}</section></>;
}
function SolutionCase({row,onChanged}:{row:Row;onChanged:()=>void}){const [detail,setDetail]=useState<Row|null>(null);const open=async()=>{const rows=await api<Row[]>(`/api/solution-cases?status=${row.status}`);setDetail(rows.find(item=>item.id===row.id)??null)};return <details className="solution-case" onToggle={e=>{if((e.currentTarget as HTMLDetailsElement).open&&!detail)void open()}}><summary><span className="schedule-glyph"><Icon name="briefcase"/></span><span><span className="category">{row.area}</span><b>{row.title}</b><small>{row.domain_name??'General workspace'} · {row.completed_actions}/{row.actions} actions done</small></span><span className={`pill ${row.status==='validated'?'ok':'neutral'}`}>{row.status}</span></summary><div className="case-body"><p>{row.objective}</p>{detail?(detail.actions??[]).map((action:Row)=><div className="case-action" key={action.id}><span><b>Phase {action.phase} · {action.title}</b><small>{action.rationale}</small><ul>{(action.instructions??[]).map((item:string)=><li key={item}>{item}</li>)}</ul></span><select value={action.status} onChange={async e=>{await patch(`/api/solution-actions/${action.id}`,{status:e.target.value});setDetail(null);await open();onChanged()}}>{['open','doing','blocked','done','dismissed'].map(value=><option key={value}>{value}</option>)}</select></div>):<Loading/>}<div className="button-row"><select value={row.status} onChange={async e=>{await patch(`/api/solution-cases/${row.id}`,{status:e.target.value});onChanged()}}>{['discovery','planned','active','waiting','validated','closed'].map(value=><option key={value}>{value}</option>)}</select><button onClick={async()=>{await post(`/api/solution-cases/${row.id}/plan`,{});setDetail(null);await open();onChanged()}}>Regenerate open playbook</button><button className="danger-button" onClick={async()=>{if(window.confirm(`Delete case “${row.title}”?`)){await api(`/api/solution-cases/${row.id}`,{method:'DELETE'});onChanged()}}}>Delete</button></div></div></details>}
...
function Icon({ name }: { name: string }) {
  const paths: Record<string, ReactNode> = {
    ...
    megaphone: <><path d="M4 11v3a2 2 0 0 0 2 2h2l2 4h3l-1.5-4L20 19V6L8 9H6a2 2 0 0 0-2 2z"/><path d="M20 10a3 3 0 0 1 0 5"/></>,
...
function parseView(): View {
  ...
  if (parts[0] === 'growth') return { name: 'growth' };
  if (parts[0] === 'ads') return { name: 'ads' };
  if (parts[0] === 'strategy') return { name: 'strategy' };
