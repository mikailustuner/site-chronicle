...
import { runIntelligenceJob } from './intelligence-jobs.js';
...
while(!stopping){
  ...
  if(Date.now()-lastRetention>86_400_000){if(config.retentionDays>0)await cleanupRetention(sql,config.retentionDays).catch(error=>console.error('Retention',error));lastRetention=Date.now()}
...
async function handle(job:Job):Promise<void>{const timer=setInterval(()=>{void refreshJobLock(sql,job.id,config.workerId).then(ok=>{if(!ok)console.error(`Job ${job.id} lock heartbeat was rejected`)})},60_000);timer.unref();try{if(job.type==='audit')await runAudit(sql,String(job.payload.auditId));else await runIntelligenceJob(sql,job);await completeJob(sql,job.id,config.workerId)}catch(error){console.error(`Job ${job.id} failed`,error);await failJob(sql,job,error,config.workerId)}finally{clearInterval(timer)}}
